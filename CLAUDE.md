...
- The top-of-board **color legend** is a matrix (customers × environment-names, cell = that env's color), rendered into the header `#legend` and shown on the board at **every drill level** (`renderLegend()` gates on `nav.level==="board"`); it's hidden on the hub/overlays. `render()` returns early off-board, so it also calls `renderLegend()` on that path to clear stale state.
