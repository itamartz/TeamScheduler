...
**All-day / multi-day span tasks:** a task is either **timed** (the original: one `date` + `start_min` + `duration_min`) or an **all-day span** (`all_day:true`) that covers whole days `date`..`end_date` (inclusive, `end_date >= date` enforced; a span carries no time, so `start_min`/`duration_min` are stored as `0`). `New-/Set-Task` take `-AllDay`/`-EndDate`; the POST route + `Set-TaskFromBody` bridge `all_day`/`end_date`, and `Set-Task` normalizes on every write (all-day ⇒ valid span + zeroed time; timed ⇒ `end_date=null` + validated time). The editor's **"יום מלא / מספר ימים"** checkbox (`#tAllDay`) swaps the time row for a span row that keeps **number-of-days** (`#tDays`) and **end date** (`#tEndDate`) in sync from the start date — `#tSpanMode` chooses which one you edit — with a live `#tSpanSummary` range line. **The day count is in WORKING days — Sun–Thu that are not holidays; Fri/Sat and any `holidayOn()` date are stepped over** (so "10 days" from a Sunday with no holidays ends on Thursday of the next week — two full work-weeks; a holiday inside the range pushes the end out by one more working day). `isWorkdayIso`/`nextWorkdayIso`/`endIsoFromDays`/`workdaysBetween` (all front-end, holiday-aware) do the math; the stored value is still a plain calendar range `date..end_date`, and `spanDayCount()` recomputes working days from it on every render (so editing holidays later re-labels existing spans). Front-end model: spans are kept **out of `taskIndex`** (which stays timed-only) in a separate `spanTasks` list; `isSpan`/`spanEnd`/`spanDayCount`/`spanCoversIso`/`spansForDayIdx` are the helpers. Rendering — the **weekly board** draws a span as a self-contained `.spanbar` pill in **every working day it covers**, each showing the task title so each day reads as its own task, plus an `N/total` working-day counter (`.spanday`) and the date range on the first covered column (`spanBarHtml`). **Holiday columns are skipped for multi-day spans** (they aren't working days), but a single-day all-day task on a holiday still shows. The **daily view** shows covering spans in an `.alldayband` above the hour grid; the **project page** and all five **print layouts** show the range / `כל היום` instead of a time. `weekStats`/`projectStats` count a span once as a **zero-hour** task; `taskPastDeadline`/`projDeadlineHtml` at-risk checks use the span's **end** date. **Duplicate:** the editor's `שכפול` button (`#tDup`, existing tasks only) calls `duplicateTask()` → `buildTaskPayload()` (shared with `saveTask`) → POST a copy → reopen the new copy for tweaking; the weekend/overlap `confirm()` guards apply to timed tasks only.

**Task description + progress:** every task carries `description` (free text, default `""`) and `percent_done` (int 0..100, default 0). `New-/Set-Task` take `-Description`/`-PercentDone` (validated 0..100); the POST route + `Set-TaskFromBody` bridge `description`/`percent_done`; absent ⇒ defaults (front end: `taskDesc(t)`, `taskProgress(t)`). The editor has a description `#tDesc` textarea and a progress slider+number (`#tProgress`/`#tProgressNum`, synced by `setProgress()`, shown in `buildTaskPayload`). Rendering: a thin green **progress bar** (`progHtml()` → `.prog`, `.done` at 100%) sits at the bottom of every task chip/daily block/span pill when `percent_done > 0`; the description rides in the hover tooltip (`tipDesc`/`tipPct`). The **project page** adds a התקדמות column (mini bar + %) and shows the description under the title (`.pjdesc`); the manager print table adds a הושלם % column.
...
- The **color legend** is a matrix (customers × environment-names, cell = that env's color), rendered into the header `#legend` and **always shown** (`renderLegend()` uses `show = true`, on every view and drill level). `render()` returns early off-board, so it calls `renderLegend()` on that path too so the legend stays populated on the hub.
